...
def check_permutation(floor_number, current_floors, previous_floors, perm, steps, diff):
    ...
    if safe_floor(current_floors[floor_number + diff] + list(perm)):
        ...
        if safe_floor(copy_floors[floor_number]):
            ...
            if not same_floors(copy_floors, previous_floors):
                ...
                if process_floor(floor_number + diff, copy_floors, current_floors, steps + 1):
                    return True
    return False


def check_permutation_up(floor_number, current_floors, previous_floors, perm, steps):
    return check_permutation(floor_number, current_floors, previous_floors, perm, steps, 1)


def check_permutation_down(floor_number, current_floors, previous_floors, perm, steps):
    return check_permutation(floor_number, current_floors, previous_floors, perm, steps, -1)


def process_floor(floor_number, current_floors, previous_floors, steps=0):
    ...
    if floor_number < 3:
        for perm in permutations(current_floors[floor_number], 2):
            # First try to bring two items up
               if check_permutation_up(floor_number, current_floors, previous_floors, perm, steps):
                return True
    ...
    if floor_number >= 0:
        for perm in current_floors[floor_number]:
            ...
            if check_permutation_down(floor_number, current_floors, previous_floors, [perm], steps):
                return True
        for perm in permutations(current_floors[floor_number], 2):
...
process_floor(elevator, floors, floors)
